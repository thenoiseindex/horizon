...
void setup() {
  ...
  size(400, 300, PDF,"MyForLooperPrnter.pdf" );
  smooth();
  noFill();
}
...
void draw() {
  ...
  if (option == 1) {
    // Option 1: Stitches
    for (float rep=0; rep<width;rep +=0.9){
    for (int x = 50; x <= width-50; x +=20+rep) {
      for (int y = 50; y <= height-50; y+=20+rep) {
    ...
    for (int x = 50; x <= width-50; x += 10+f) {
      for (int y = 50; y <= height-50; y+=10+f) {
